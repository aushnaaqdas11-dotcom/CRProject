...
import { assignerAPI } from '../services/apiService';
import AsyncStorage from '@react-native-async-storage/async-storage';

const AssignerDashboard = ({ navigation }) => {
  ...
  const loadDashboard = async () => {
    try {
      ...
      const response = await assignerAPI.getDashboard();
      ...
      console.log('Assigner dashboard error:', error);
      Alert.alert('Error', 'Failed to load assigner dashboard');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Requests to Assign</Text>
          {dashboardData?.requests && dashboardData.requests.length > 0 ? (
            ...
            <Text style={styles.noDataText}>No requests to assign</Text>
          )}
        </View>
      </ScrollView>
